CREATE DATABASE supplier_aprameya;
USE supplier_aprameya;
...
/* Find the pnames of parts supplied by Acme Widget and by no one else*/
SELECT p.pname
FROM Parts p
JOIN Catalog c ON p.pid=c.pid
JOIN Supplier s ON s.sid=c.sid
GROUP BY p.pname
HAVING count(DISTINCT CASE WHEN s.sname="Acme Widget"THEN s.sid END)=1
 AND count(DISTINCT s.sid)=1;
 /* Find the sides of Suppliers who charge more for some part than the average cost of that part*/
SELECT DISTINCT c.sid
From Catalog c
WHERE c.cost>(SELECT AVG(c2.cost)
           FROM Catalog c2
           WHERE c2.pid=c.pid);
/* For each part,find the sname of the supplier who chargers the most.*/
